module Day5 exposing (parsedInput, part1, part1Output)
...
parsedInput =
    inputText |> String.split "," |> List.filterMap String.toInt |> Array.fromList
...
part1 : Int -> Int -> { memory : Array Int, output : Int } -> { memory : Array Int, output : Int }
part1 input index state =
    let
        memory =
            state.memory

        { opCode, firstParameter, secondParameter, thirdParameter } =
            Array.get index memory |> Maybe.withDefault 0 |> decodeInstruction |> Debug.log "s"

        value0 =
            getMemory firstParameter (index + 1) memory |> Debug.log "value0"

        value1 =
            getMemory secondParameter (index + 2) memory |> Debug.log "value1"

        value2 =
            unsafeGet (index + 3) memory
    in
    if opCode == 1 then
        { state | memory = Array.set value2 (value0 + value1) memory } |> part1 input (index + 4)
        ...
        { state | output = value0 } |> part1 input (index + 2)
...
getMemory mode index values =
    ...
    case mode of
        PositionMode ->
            Array.get positionValue values |> Maybe.withDefault 0
